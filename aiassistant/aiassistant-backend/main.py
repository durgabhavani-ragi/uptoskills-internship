...
load_dotenv()
setup_logging()
logger = logging.getLogger("aiassistant.main")
...
app = FastAPI(
    title="SkillNova AIAssistant",
    version="1.0.0",
    description=(
        "Agentic RAG chatbot that ships as the AIAssistant across the "
        "SkillNova platform — available to every role (SUPER_ADMIN, "
        "ADMIN, MENTOR, INTERN)."
    ),
    lifespan=lifespan,
)
...
@app.get("/")
def root():
    return {"status": "running", "service": "skillnova-aiassistant"}


@app.get("/api/aiassistant/session")
def create_session():
    return {"session_id": str(uuid.uuid4())}


@app.get("/api/aiassistant/health")
def health():
    return {
        "status": "ok",
        "service": "AIAssistant",
...
# ─────────────────────────────────────────────────────────────────────
@app.post("/api/aiassistant/chat", response_model=ChatResponse)
async def chat(req: ChatRequest):
    ...
    # Allow callers to pass either `role` or `user_role`. Prefer user_role.
    user_role = req.user_role or req.role or "Intern"
...
# ─────────────────────────────────────────────────────────────────────
@app.post("/api/aiassistant/chat/stream")
